...
Parser.prototype._star = function () {
    ...
    A[A.length - 1].addTransition(0, end);
    A[A.length - 1].addTransition(0, A[0]);
    A.unshift(start);
    A.push(end);
...
Parser.prototype._union = function () {
    ...
    let end = new State();
    start.addTransition(0, A[0]);
    start.addTransition(0, B[0]);

    A[A.length - 1].addTransition(0, end);
    B[B.length - 1].addTransition(0, end);
    A = A.concat(B);
    A.unshift(start);
    A.push(end);
...
// TODO dump NFATable/DFATable for debug
